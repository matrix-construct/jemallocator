...
/// Adjust the layout's size and alignment based on platform requirements prior to
/// calls into jemalloc.
///
/// # Safety
///
/// This function only makes certain limited and efficient adjustments to the input layout.
/// It is not a general sanitizer. The input layout must still construct a valid `Layout`
/// which would not `Result` in `Err`, as the construction here is unchecked.
#[inline]
